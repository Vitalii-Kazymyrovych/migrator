...
class HardhatsNotificationsMigratorTest {
    ...
    void addsLatLongDefaults() {
        ...
        row.put("stream_id", 8);
        row.put("va_id", 9);
        when(source.queryForList("SELECT * FROM hardhats_notifications")).thenReturn(List.of(row));

        new HardhatsNotificationsMigrator(source, destination).migrate();
        ...
        assertEquals(null, transformed.get("longitude"));
    }
}
